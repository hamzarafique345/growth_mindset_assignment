...
#         if numeric_columns:
#             column_to_plot = st.selectbox(f"Select a column to visualize for {uploaded_file.name}", numeric_columns, key=f"col_{uploaded_file.name}")

#             # Create a histogram plot
#             fig, ax = plt.subplots(figsize=(10, 5))
#             sns.histplot(df[column_to_plot], kde=True, bins=30, ax=ax)
#             ax.set_title(f"Distribution of {column_to_plot}")
#             st.pyplot(fig)
#         else:
#             st.warning(f"No numeric columns found in {uploaded_file.name} for visualization.")
